...
- **`linkedSignal` for `suppressedLayoutIds`.** When the user deletes a row, the layout is hidden until the parent emits a new `layouts` input (`linkedSignal` resets automatically).
